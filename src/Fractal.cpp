...
void Fractal::add_colour_range(double start_point, double end_point, const Rgb& colour)
{
	...
	{
		pixels_with_iterations =
			_cumulative_histogram[end_iteration - 1] -
			_cumulative_histogram[start_iteration];
	}
	else
	{
		pixels_with_iterations =
			_cumulative_histogram[end_iteration] -
			_cumulative_histogram[start_iteration];
	}

	std::tuple<int, int, int, Rgb> range = std::make_tuple(
		start_iteration, end_iteration, pixels_with_iterations, colour);
...
Fractal::Fractal(int width, int height, int max_iterations) 
	: _width(width), _height(height), _max_iterations(max_iterations)
{
...
void Fractal::compute()
{
	...
	for (int y = 0; y < _height; ++y)
	{
		for (int x = 0; x < _width; ++x)
		{
			...
			iterations = Mandelbrot::get_iterations(
				xy_fractal.first, xy_fractal.second, _max_iterations);
	...
	for (int i = 0; i < _max_iterations; ++i)
	{
		...
		{
			_cumulative_histogram[i] =
				_histogram[i] + _cumulative_histogram[i - 1];
		}
...
void Fractal::draw()
{
	...
	for (int y = 0; y < _height; ++y)
	{
		for (int x = 0; x < _width; ++x)
		{
			...
			std::tuple<int, int, int, Rgb> range_info = 
				colour_ranges[colour_range];

			std::tuple<int, int, int, Rgb> next_range_info =
				colour_ranges[colour_range + 1];
			...
			if (iterations != _max_iterations)
			{
				...
				for (int i = colour_start_iterations; i <= iterations; ++i)
					hue += (double)_histogram[i] / 
						_cumulative_histogram[_max_iterations - 1];
